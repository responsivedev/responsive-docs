...
transactional semantics without a performance penalty.
...
these databases operate well in the context of kafka Streams. The next few
sections examine some of the concepts we've implemented.
...
the [State Store API Reference](/java-sdk/stores) for more details.
...
- We implement [Async Processing](/java-sdk/async-processing) to concurrently
...
Both MongoDB and ScyllaDB allow us to partition data independently of the
...
data in the remote stores directly.
