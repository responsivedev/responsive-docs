...
This page covers the Responsive Async Processor: its benefits, its limitations, how to use it, and how to tune it.

The Async Processor is available with Responsive client **v0.24.0** onward.
:::
...
While async processing can help improve performance of any application with a large keyspace, it was built to shine when processors include a long-running step such as remote state store requests or generic RPCs. The classic solution for achieving high throughput when you are bottlenecked by network latency is to increase the parallelism of the calls to the target system, which is exactly what the async processor does for Kafka Streams. 

This makes the Responsive Async Processor the perfect tool to complement Responsive state stores, meaning you don’t need to sacrifice performance in order to gain the availability and elasticity benefits of remote storage with Kafka Streams!

## Expected Performance Gains

As with all real-world systems, performance depends heavily on the specific use case and parameters. However, in all of our experiments, async processing showed an improvement over regular Kafka Streams when a topology includes a processor which makes a remote call. 

<img
    alt={"Responsive Asyc Processor Peformancs"}
    src={require('/static/img/docs/reference/async-processing/async-processing-perf.png').default}
    className={'px-[10%] mb-4'} />


To give an idea of the benefits of using the async processor when your topology includes higher latency external calls, we ran a test that injected a variable latency in our remote storage client while running a simple application that would deduplicate events over a window. A couple of things stand out from the results above: 

1. The greater the remote latency, the bigger the impact of async. At 2 ms round trip latency, throughput improves by 5.5x. At 1 ms round trip latency, throughput improves by 3x.
2. The throughput with async processing enabled is stable even with varying latency.

:::note
The async processor makes the round trip network latency a non-factor in achieving high throughput. However, using the async processor will generally place more load on the remote system, which in turn may get saturated, which in turns will drive up the latencies, which will bring down the throughput gains of the async processor. 

Even so, the async processor performed at a steady 30% better even when the remote system was at its limit in our testing.
:::

We are continuing to test and tune the async processor, and will share more results as we have them. 
...
You will know if this is not set up correctly by an immediate `IllegalStateException` upon startup.
...
Although we always advise increasing the commit interval for EOS applications, it is especially to do so when enabling the async processing framework if you want to maximize its potential. If your application has only a few subtopologies (one or two), you can probably bump up the `commit.interval.ms` to 30s (which is the default for at-least-once-semantics). If your application has a larger number of subtopologies, the end-to-end latency will accrue across them, so it may be good to divide the 30s recommendation by a factor of the subtopology count. 
...
Eventually, the Responsive Controller will tune the async processor for you, but if you have questions hop on to our discord and [ask us](https://discord.gg/jEk8JvjJrg)!
