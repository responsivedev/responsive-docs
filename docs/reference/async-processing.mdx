...
This page covers the Async Processor: its benefits, its limitations, how to use it, and how to tune it.

The Async Processor is available with Responsive client **v0.25.0** onward.
:::
...
2. The throughput with async processing enabled is stable even with varying remote latency. The processor was ultimately bottlenecked on the resources available on the client side!

:::warning
The async processor makes the round trip network latency a non-factor in achieving high throughput. However, using the async processor will generally place more load on the remote system, which in turn may get saturated, which in turn will drive up the latencies, which will mean the throughput gains of the async processor will be below the theoretical maximum.
...
|--------|--------------|
| `commit.interval.ms` | The semantics of the commit interval don't change when changing the async processor, but a low value will mean you get less of a benefit from using the async processor | 
