...
Responsive ttl is most effective for non-historical data, as the data is actively expired based on wallclock time
rather than stream-time. Backfilling historical data may build up and require more storage until the ttl kicks in.
Note, however, that results are always filtered in the client layer based on stream-time, so that you can trust
in the ttl semantics and get reliable, deterministic results.
