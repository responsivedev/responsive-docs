...
The Responsive CLI runs a proxy server that handles communication with storage services and
control plane services that run in the Responsive network. Today, it supports the following
operations:
